...
As interpolações referidas ocorrem a partir das temperaturas em cada volume de controle do cálculo por volumes
finitos, sendo utilizadas tabelas previamente geradas de coeficientes utilizados na equação de difusão de nêutrons.
Valores de seções de choque e coeficientes de difusão para dois grupos de nêutrons estão tabelados para quatro
diferentes temperaturas e os valores destes coeficientes para cada célula são obtidos por interpolações lineares.

A variação de densidades dos materiais modelados é ignorada neste trabalho. Entretanto, é necessário salientar que tal variação nas densidades
tem implicação direta nos valores de seções de choque macroscópicas e que tal simplificação conceitual pode levar
a diferenças importantes nos resultados dos cálculos.
...
Talvez possa parecer corriqueira tal observação nos dias de hoje. Entretanto, há pouco mais de trinta
...
Há diferentes técnicas para evitar condições de corrida. Dentre elas, uma das mais simples e
...
%seção \ref{sec:intsl}.
Nesta subseção, a análise se restringe às vantagens do acesso ao
código-fonte em sistemas abertos.
...
A formulação utilizada para a solução das equações diferenciais que modelam o problema multi-física desta
...
\item \textbf{Seções de choque macroscópicas}: a geração de seções de choque deve ser feita por algum código externo. Os
  ...
  desta funcionalidade ainda é incipiente. Outra iniciativa recente na geração de seções de choque
  a partir de dados nucleares, esta encabeçada pelo
  \textit{Los Alamos National Laboratory}, é o lançamento do código NJOY \cite{NJOY2016} sob licença livre \cite{BSD3}, com o objetivo
    de estender o desenvolvimento de novas funcionalidades à toda comunidade nuclear. O website desta nova
    versão é https://njoy.github.io/about/index.html.
...
ao mesmo tempo, capacidade de entrada de dados paramétrica e visualização por interface gráfica. O Gmsh permite
...
Este \textit{solver} resolve problemas em estado estacionário (não são considerados
termos variando no tempo) e para
...
O sistema a ser resolvido consiste em uma representação em estado estacionário de um elemento
combustível do tipo TRIGA. Este sistema pode ser resolvido utilizando-se a equação de difusão
para solução do fluxo neutrônico e, a partir deste, calculada a potência nominal. Um \textit{software}
capaz de resolver este conjunto de equações diferenciais na mesma malha, ou seja, para o mesmo domínio
utilizado pelo \textit{OpenFOAM}, seria uma escolha ideal.
...
& + \sum_{g'\neq g}^{G} \Sigma_{sg'\rightarrow g}(\mathbf{x}) . \phi_{g'}(\mathbf{x})
...
\begin{table}[htb]
\centering
%\caption[Nomenclatura equação de difusão de nêutrons.]{onde,}
\label{tab:eq-dif}
\begin{tabular}{ll}
...
pelo \textit{milonga}. Isso significa que, como a cada cálculo executado pelo \textit{milonga}
as seções de choque são efetivamente constantes, é fisicamente realístico interpolar as seções
de choque linearmente para o conjunto de temperaturas disponível a cada iteração.

Na equação \ref{eq:difusao} assume-se que todos os coeficientes de difusão utilizados são
...
Os coeficientes da equação de difusão para um material qualquer são apresentadas nas linhas 10-16.
...
\begin{figure}[htb]
  \caption{Exemplo de arquivo de entrada básico do milonga. }
...
% -------------------------------------------------------------------------------------------------------------
\subsection{Algoritmo do sistema termo-hidráulico}
\label{subsec:th}
...
\item \textbf{Inicialização:} Na inicialização do \textit{OpenFOAM}, é lido o valor inicial da potência para
  a simulação caso o arquivo de definição de potência esteja disponível. Caso contrário, a potência é inicializada
...
% -------------------------------------------------------------------------------------------------------------
\subsection{Algoritmo do sistema neutrônico}
...
de execuções fixo, cujo valor é de mil chamadas da termo-hidráulica, suficiente para o caso de prova
...
\begin{itemize}
\item \textbf{Acoplamento termo-hidráulico:} A execução dos cálculos neutrônicos se inicia com a leitura do semáforo da
...
% -------------------------------------------------------------------------------------------------------
\subsection{Implementação: visão geral do código-fonte.}
\label{subsec:detth}
...
a região sólida. É nele que o campo escalar volumétrico \textbf{``Q''} é utilizado. Um fragmento
...
8 e 9 são declaradas as estruturas que representam os arquivos em memória compartilhada. Há, portanto, três
...
Finalmente, nas linhas 11 e 12 é feito o que se convencionou chamar ``coerção'' em Ciência da Computação.
...
são, por vezes, uma forma útil de fazer a conexão entre a metodologia e a prática.
