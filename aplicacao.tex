...
utilizadas e as regiões correspondentes. As superfícies definidas como simétricas
estão em vermelho
...
para os parâmetros das respectivas condições de contorno. Como resultado do pré-processamento
entre regiões, são obtidas condições de contorno que implementam a troca
conjugada de calor entre as distintas regiões.

Na tabela \ref{tab:cc-superf} são apresentadas as condições de contorno utilizadas
nas superfícies entre regiões para a temperatura. É possível perceber que todas as superfícies,
...
propriedades termofísicas dos respectivos materiais previamente definidas para o cálculo
em tempo de simulação, do fluxo de calor entre as regiões. As propriedades termofísicas
dos materiais são dadas na tabela \ref{tab:termofisica}.

\begin{table}[]
\centering
\caption{Propriedades termofísicas dos materiais}
...
\hline
Combustível  & 6280                                                               & \begin{tabular}[c]{@{}l@{}}0.892+\\ 4.44361x10\end{tabular}              & \begin{tabular}[c]{@{}l@{}}22.872-\\ 4.3131e-3\end{tabular}              & -                                                                     \\
\hline
    Revestimento & 2705                                                               & \begin{tabular}[c]{@{}l@{}}0.294+\\ 6.196e-4\end{tabular}                & \begin{tabular}[c]{@{}l@{}}223.7+\\ 4.756e-3+\\ 555+\\ 4444\end{tabular} & -                                                                     \\
\hline
...
\label{tab:cc-superf}
\begin{tabular}{llll}
  \textbf{Região}                   & Superfície    & Condição de Contorno                  & Campo                  \\
  \hline
  \textbf{Combustível}                   & \textit{fuel\_to\_cladding}    & compressible::turbulentCoupledBaffleMixed                  & T                  \\
  \hline
  \multirow{2}{*}{\textbf{Revestimento}} & \textit{cladding\_to\_fuel}    & \multirow{2}{*}{compressible::turbulentCoupledBaffleMixed} & \multirow{2}{*}{T} \\
  & \textit{cladding\_to\_coolant} &                                                            &                    \\
  \hline
  \textbf{Refrigerante}                  & \textit{coolant\_to\_cladding} & compressible::turbulentCoupledBaffleMixed                  & T                 
\end{tabular}
\end{table}
...
dos campos internos. Entretanto, isto não desobrigado o usuário de explicitamente definí-las como \textit{calculated}.
...
Uma vez definidas as características físicas do modelo e as condições iniciais e de contorno, é
possível dizer que o problema está fechado e é possível iniciar os cálculos propriamente ditos.
Entretanto, de modo a iniciar os cálculos, são necessárias, além das configurações relativas
ao problema físico, as definições relativas aos esquemas de discretização utilizados e aos
algoritmos numéricos usados. Complementando a breve introdução ao método de volumes finitos
dada no capítulo \ref{chap:teoria} desta tese, são apresentados na tabela \ref{tab:discretizacao}
os esquemas de discretização
utilizados nos cálculos acoplados e não acoplados.
...
\textbf{Região}      & gradiente                                                   & divergente                                                              & laplaciano                                                           & interpolação & \begin{tabular}[c]{@{}c@{}}normal à\\ face da célula\end{tabular} \\ \hline
