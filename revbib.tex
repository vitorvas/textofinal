...
desenvolvida tendo em vista as aplicações nucleares é o código milonga \cite{Theler2014b}.
...
de multifísica já são uma realidade, e não mais uma aposta ou uma perspectiva futura.
...
neutrônico e termo-hidráulico, propõe o uso de redes neuronais treinadas com dados de
...
termo-hidráulica, Klaus Jareteg extendeu seu trabalho à simulação de elementos combustíveis de
reatores do tipo PWR \cite{Jareteg2015}. Neste trabalho, é proposto um mapeamento entre os valores
calculados de seções de choque - mais uma vez utilizando-se do \textit{software Serpent2} - gerados
...
apresentado pelo autor na sua tese de doutorado \cite{Jareteg2012}. Cabe também ressaltar que os trabalhos
...
fim usando uma variedade de aplicações distintas que se comunicam fazendo o cálculo multifísica. Algumas
das aplicações são de neutrônicca, termo-hidráulica, avaliação de desempenho de combustíveis e química
...
da distribuição de potência nos materiais num sistema de transportes de neutrons baseado
...
do código MCNP. Também faz uso de scripts em linguagem \textit{Python} para
tratamento do mapeamento além de estruturas de dados próprias dessa linguagem de programação
para conseguir inserções e remoções em memórias a custo $O(1)$ \cite[Seção~9.2]{Graham1994}.
...
merecem mensões não apenas pela forma como estão relacionados com o trabalho desenvolvido nesta tese, mas pela
...
acoplado \cite{Bennett2016}. Dois de seus autores são os mesmos do trabalho considerado referência em termos
de acoplamento neutrônico e termo-hidráulico \cite{Ivanov2007}. Neste acoplamento, são usados um código
de Monte Carlo (MCNP6) para os cálculos neutrônicos e um código de sub-canais (CTF). Apesar de não apresentarem
inovações grandes em relações a acoplamentos anteriores, há duas coisas para as quais deve ser chamada a atenção.
O MCNP6 é capaz de gerar seções de choque \textit{on-the-fly}, uma característica desenvolvida com objetivo
de oferecer suporte ao acoplamento termo-hidráulico. Há alguns anos, o uso de códigos de Monte Carlo para
acoplamento era impensável. Entretanto, nove anos depois do trabalho referência em acoplamento, não só temos
diversos exemplos do uso de códigos de Monte Carlo acoplados, como dois dos principais deles (MCNP e Serpent)
já possuem por definição de projeto, formas de serem acoplados. A segunda razão que vale mencionar neste
trabalho, pode passar despercebida: como já mencionado, são nove anos entre dois trabalhos no mesmo tempo.
Isso serve como um bom exemplo do que é hoje a execução de cálculos acoplados e a insistência em
investigar formas de fazê-la cada vez mais robusta. A multifísica é hoje uma realidade.
...
não reside na tentativa de demonstrar a importância dos cálculos multifísica - espera-se que, neste ponto,
...
Neste trabalho é usado o código \textit{OpenMC} \cite{Romano2013}. Desenvolvido no \textit{Massachusetts
  Institute of Technology} (MIT), é um código aberto. Oferecido com uma licença específica do próprio
MIT, é garantido, sem custo, a qualquer pessoa a obter o código-fonte desse \textit{software} e sua
documentação associada, o usá-lo sem restrição e a permitir que as pessoas a quem seja fornecida
copia, os mesmos direitos. E é este o ponto a que se deseja dar ênfase: diferentemente da maioria
dos \textit{softwares} usados e disponíveis na indústria nuclear - leia-se indústria também englobando
as pesquisas científicas - este é completamente aberto e livre. Um sinal de que a mudança de paradigmas na
...
Seu desenvolvimento não aplica do desenvolvimento de \textit{software} aberto de forma incidental, pelo
contrário, o seu objetivo é ser um sistema aberto, flexível, robusto e de uso geral, ao contrário das
soluções existentes baseadas em sistemas fechados definidos para conjuntos de reatores específicos.
No paper citado, os autores explicam os conceitos do projeto e desenvolvimento do \textit{Cyclus}, com
...
Concluindo, nesse capítulo tentou-se dar uma visão geral sobre o que é \textit{software} livre e do
